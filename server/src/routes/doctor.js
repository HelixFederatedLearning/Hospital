...
// import { trainNowForHospital } from '../worker.js';
...
// router.post('/fl/train-now', requireAuth, async (req, res) => {
//   try {
//     const { hospitalId } = req.user;
//     const result = await trainNowForHospital(hospitalId);
//     return res.json(result);
//   } catch (e) {
//     console.error('train-now error:', e);
//     return res.status(500).json({ error: e.message });
//   }
// });

// router.get('/fl/status', requireAuth, async (req, res) => {
//   const { hospitalId } = req.user;
//   const [queued, used] = await Promise.all([
//     TrainSample.countDocuments({ hospitalId, status: 'queued' }),
//     TrainSample.countDocuments({ hospitalId, status: 'used' })
//   ]);
//   res.json({ hospitalId, queued, used });
// });
...
// // ---------- Upload training images + metadata ----------
...
//         hospitalId: req.user.hospitalId,              // <—— NEW (multi-hospital)
...
router.post('/fl/train-now', requireAuth, async (req, res) => {
  try {
    const { hospitalId } = req.user; // embedded in JWT on login
    const result = await trainNowForHospital(hospitalId);
    return res.json(result);
  } catch (e) {
    console.error('train-now error:', e);
  ...
  try {
    ...
    for (const f of req.files) {
      ...
      docs.push({
        userId: req.user.sub,
        doctorId: doctor?._id || null,
        hospitalId: req.user.hospitalId,        // IMPORTANT: multi-hospital key
        filename: f.originalname,
        classLabel: String(m.classLabel || '').trim(),
        confidence: Number(m.confidence ?? 0),
        path: f.path,
